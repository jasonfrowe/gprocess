...
def genchain(x,data,beta,niter,func,loglikelihood,lprior,mcmcfunc,buffer=[],corbeta=1,pbar=0): 
    "Generate Markov Chain"
    chain=[]                                  #Initialize list to hold chain values
    accept=[]                                 #Track our acceptance rate
    chain.append(x)                           #Step 1: start the chain
    accept.append((0,0))
    llx=loglikelihood(func,x,data)    #pre-compute the log-likelihood for Step 3
    llx=llx+lprior(x)                         #Don't forget your priors!
    
    if pbar > 0:
        bar = progressbar.ProgressBar(max_value=niter-1,term_width=80)
        updatefreq=floor(min(len(x)*2,niter))
    
    for i in range(0,niter):
        x,llx,ac = mcmcfunc(x,llx,beta,data,func,loglikelihood,lprior,buffer,corbeta)
        chain.append(x)
        accept.append(ac)
